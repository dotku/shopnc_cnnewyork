...
class buyModel {
    ...
    /**
     * 取得店铺运费(使用售卖区域的商品运费不会计算，但会返回模板信息)
     * 先将免运费的店铺运费置0，然后算出店铺里没使用售卖区域的商品运费之和 ，存到iscalced下标中
     * 然后再计算使用售卖区域的信息(array(店铺ID=>array(售卖区域ID=>购买数量))，放到nocalced下标里
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        //然后算出店铺里没使用售卖区域(优惠套装商品除外)的商品运费之和(格式:店铺ID=>运费)
        ...
        //最后再计算使用售卖区域的信息(店铺ID，售卖区域ID，购买数量),使用使用相同售卖区域的商品数量累加
        $freight_list = array();
    ...
    /**
     * 根据地区选择计算出所有店铺最终运费
     * @param array $freight_list 运费信息(店铺ID，运费，售卖区域ID，购买数量)
    ...
    public function calcStoreFreight($freight_list, $city_id) {
		...
		//使用售卖区域的信息(array(店铺ID=>array(售卖区域ID=>购买数量))
		$nocalced_list = $freight_list['nocalced'];

		//然后计算使用运费售卖区域的在该$city_id时的运费值
		if (!empty($nocalced_list) && is_array($nocalced_list)) {
		    //如果有商品使用的售卖区域，先计算这些商品的运费总金额
