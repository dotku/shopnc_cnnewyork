...
$html_title = '运维舫&nbsp;SHOPNC.CLUB&nbsp;电商系统 安装程序';

$html_header = <<<EOF
<div class="header">
  <div class="layout">
    <div class="title">
      <h5>运维舫&nbsp;SHOPNC.CLUB&nbsp;电商系统 安装程序</h5>
      <h2>系统安装向导</h2>
    </div>
    <div class="version">版本: 2015.10.29</div>
  </div>
</div>
...
$html_footer = <<<EOF
<div class="footer">
  <h6><a href="http://www.shopnc.club" target="_blank">程序来源于shopnc.club</a></h6>
</div>
EOF;
...
function step3(&$install_error,&$install_recover){
	...
	$db->query("update `{$db_prefix}goods` set is_own_shop = 1 where store_id = 1");
	$db->query("update `{$db_prefix}goods_common` set is_own_shop = 1 where store_id = 1");
	$db->query("update `{$db_prefix}store` set is_own_shop = 1 where store_id = 1");
	$db->query("update `{$db_prefix}store` set bind_all_gc = 1 where store_id = 1");
    if ($_POST['demo_data'] == '1'){
        $db->query("UPDATE {$db_prefix}store SET `live_store_name`='运维舫',`live_store_address`='中国',`live_store_tel`='400-80000000',`live_store_bus`='33路' WHERE `store_id`=1");
