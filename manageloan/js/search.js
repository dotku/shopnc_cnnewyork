jQuery(function () {
    ...
    var inNumOrDouble = function (str) {
        try {
            eval(str + " * 0");
            return true;
        } catch (e) {
    ...
    var trimInputFormats = function () {
        $.each([$("#amountInput"), $("#periodInput")], function () {
            $(this).val($(this).val().replace(/\D|^0/g, ''));
            $(this).val($.trim($(this).val()));
            if (!inNumOrDouble($(this).val())) {
                $(this).val(""); 
            }
    ...
    var clearSonSearchCondition=function(){
    	$("#repaymentModelSer").val("");
    	$("#guaranteeModelSer").val("");
    	$("#orderModeIdSer").val("");
    	$("#repaymentModelStr").val("");
    	$("#guaranteeModelStr").val("");
    	$("#withSonSearch").val("");
    };
    //-------------------------------------输入处理及初始化
    var inputs = null;
    try {
        eval("inputs = postedInputs;");
    } catch (e) {

    }
    if (!inputs) {
        inputs = {amountValue: 10, periodValue: 12};
    }


    var initInputValues = function () {
        jQuery("#amountInput").val(inputs.amountValue + "万元");
        jQuery("#periodInput").val(inputs.periodValue + "个月");
    };

    initInputValues();

    //贷款金额下拉有5、10、20、30、50、100万元 , 输入金额大于0小于1000的整数
    var amountSelectList = [5, 10, 20, 30, 50, 100];

    
    //默认选择10万元
    inputs.amountValueDefault = 10;
    //贷款期限下拉有3、6、12、24、36个月 ,输入月份大于0小于36的整数
    var periodSelectList = [3, 6, 12, 24, 36];
    ...
    jQuery("#amountInput").change(function () {
        inputs.amountValue = jQuery(this).val();
    }).focus(function () {
        jQuery("#amountInput").val(inputs.amountValue);
    });

    jQuery("#periodInput").change(function () {
        inputs.periodValue = jQuery(this).val();
    }).focus(function () {
        jQuery("#periodInput").val(inputs.periodValue);
    });


    //填充金额选择列表
    jQuery("#amountSelectContainer").empty();
    jQuery.each(amountSelectList, function (index, value) {
        var html = "<li class='amountSelectItem' id='amountSelectItem_" + value + "'  data-value='" + value + "' data-show='" + value + "万元'>" + value + "万元</li>";
        jQuery("#amountSelectContainer").append(jQuery(html));
    });


    //金额选择项添加事件
        jQuery(".amountSelectItem").click(function () {


        //给输入框复制
        jQuery("#amountInput").val(jQuery(this).attr('data-show'));

        //给输入复制
        inputs.amountValue = jQuery(this).attr('data-value');

        //隐藏下拉
        jQuery("#amountSelectContainer").hide();
    ...
    //金额下拉框按钮 添加事件
    jQuery('.amountSelectTrigger').click(function () {
        jQuery("#amountSelectContainer").toggle();
    });

    //填充期限选择列表
    jQuery("#periodSelectContainer").empty();
    jQuery.each(periodSelectList, function (index, value) {
        var html = "<li class='periodSelectItem' id='periodSelectItem_" + value + "'  data-value='" + value + "' data-show='" + value + "个月'>" + value + "个月</li>";
        jQuery("#periodSelectContainer").append(jQuery(html));
    });

    jQuery(".periodSelectItem").click(function () {

        jQuery("#periodInput").val(jQuery(this).attr('data-show'));
        inputs.periodValue = jQuery(this).attr('data-value');
        jQuery("#periodSelectContainer").hide();
    });
    jQuery('.periodSelectTrigger').click(function () {
        jQuery("#periodSelectContainer").toggle();
    });
    jQuery('.provinceSelectTrigger').click(function () {
        jQuery("#provinceSelectContainer").toggle();
    });
    jQuery('.citySelectTrigger').click(function () {
        jQuery("#citySelectContainer").toggle();
    });

    jQuery("#searchBt").click(function () {

        clearSonSearchCondition();
        mainSearCondition();
     });

    var mainSearCondition=function(){
    	trimInputFormats();
    	inputs.amountValue = jQuery("#amountInput").val();
        inputs.periodValue = jQuery("#periodInput").val();
        ...
        var debug = false;
        if (debug) {
            alert(inputs.amountValue + " 万元   " + inputs.periodValue + " 个月");
        }

        jQuery("#form_amount").val(inputs.amountValue);
        jQuery("#form_period").val(inputs.periodValue);

        if (!debug)
            jQuery("#hiddenSearchForm").submit();
    };


    var selectContainers = ['amountSelectContainer', 'periodSelectContainer'];
    jQuery.each(selectContainers, function (index, value) {
        jQuery("#" + value).mouseleave(function () {
            jQuery(this).hide();
        });
    });
    $(".searchGuranteeOrRepaymentLis").click(function (e) {
    	e.stopPropagation();
        $(".searchTypeContainer").not($(this).find("ul")).hide();
       $(this).find("ul").toggle();
 });
 $(".selectItemForProList").click(function (e) {
 		e.stopPropagation();
        //取值
        var valueStr = $(this).attr('data-show');
        var value = $(this).attr('data-value');
        var name=$(this).attr('data-name');
        //给输入框复制
        $("#"+name+"Input").html(valueStr);
        $("#"+name+"Str").val(valueStr);
		$("#"+name+"Ser").val(value);
        //隐藏下拉
        $(".searchTypeContainer").hide();
        $("#withSonSearch").val(1);
        mainSearCondition();
    });
    $("body").click(function(){
